...
export function Calendar(){
    ...
    if (monthNumber <= 7 && monthNumber % 2 == 1 || monthNumber > 7 && monthNumber % 2 == 0 || monthNumber == 0)
        isPast31 = true

    switch (today) {
        case 1:
            if (isPast31)
                listDay = [30, 31, today, today+1, today+2]
            else
                listDay = [29, 30, today, today+1, today+2]
            break
        case 2:
            if (isPast31)
                listDay = [31, 1, today, today+1, today+2]
            else
                listDay = [30, 1, today, today+1, today+2]
            break
        case 29:
            if (!is31)
                listDay = [today-2, today-1, today, today+1, 1]
            break
        case 30:
            if (is31)
                listDay = [today-2, today-1, today, today+1, 1]
